...
import 'package:revised_quickassist/UI/AllPages/MyWorkTab/MyWork.dart';
...
class PastContracts extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => PastContractsState();
}

class PastContractsState extends State<PastContracts> {
  double width, height;
  final bloc = new MyworkBloc();

  @override
  void initState() {
    if (pastContracts.getResponse == null) {
      bloc.add(getPastWork());
    }
    super.initState();
  }
  ...
  Widget displayScreen() {
    var response = pastContracts.getResponse;
    return pastContracts.getCode != 200
        ? SomethingWentWrong()
        : response.length == 0
            ? Align(
                  ...
                  return Stack(
                    children: [
                      ...
                      Container(
                        ...
                        child: Text(
                          response[response.length - index - 1]["status"]
                                      .toString() ==
                                  "Cancelled"
                              ? "Cancelled"
                              : response[response.length - index - 1]["status"]
